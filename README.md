...
> 当前发布版本：v2.33.1 后台更新与升级提醒优化（版本号 v2.33.1）
...
**新增功能**
- **后台自动下载**：可在系统设置中开启空闲时自动下载安装包，默认关闭；关闭升级窗口后仍会继续下载，完成后由用户确认安装。

**体验优化**
- **升级提醒**：发现新版本时，首页显示绿色升级图标，鼠标悬停查看提示，点击后打开升级窗口，减少操作打扰。
- **后台检查**：程序运行期间每 4 小时检查新版本，网络异常时自动重试；更新提醒在重启后保留，手动检查无需等待。
- **安装体验**：macOS 支持保留已下载的安装包，并在下载完成后确认安装；招聘任务运行期间提示稍后安装，安装失败后可重试。
...
**体验优化**
- **独立工具体积精简**：减小 Windows 独立学历核验工具体积，降低下载和存储占用。

**问题修复**
- **Chrome 窗口复用**：学信网批量核验统一复用同一 Chrome 窗口，避免标签页创建异常时偶发启动多个浏览器窗口。
