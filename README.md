...
> 当前发布版本：v2.30 招聘工作台与页面响应优化（版本号 v2.30）
...
- **招聘工作台**: 首页按岗位汇总待核实、待复核和待联系候选人，每人只进入当前最高优先级任务；同时显示最近扫描、API Key、Chrome 和本地数据状态，并提供常用工作入口
...
- **外部候选人导入**: 支持单份或批量导入猎头、内推等渠道的本地简历，按归属岗位完成筛选评分，并可选用 AI 增强画像识别和简历评估；导入后可继续复核、编辑、跟进和导出
