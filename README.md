...
- **打招呼队列**: 对符合要求的候选人集中排队发送，支持单人或多选加入队列、暂停和失败重试
...
- **结果更新**：结果页按推荐候选人、待复核、淘汰记录分类；发送结果待确认属于沟通状态，不改变候选人的筛选分类，但会阻止重复加入打招呼队列。普通待定只保留最近一次完整扫描结果，首次普通淘汰只进入运行摘要，曾经推荐、AI 淘汰或已有业务记录的候选人继续保留供复核
