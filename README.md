...
> 当前发布版本：v2.24.1 访问保护与运行记录完善（版本号 v2.24.1）
...
- **结果更新**：结果页按推荐候选人、人工确认、待复核、淘汰记录分类；55-64 分的普通待定候选人可在人工确认后加入联系清单，评分和推荐指数保持不变。仅 AI 评估失败但规则分和资格已通过时，也可基于现有资料人工批准联系，不强制重跑 AI。发送待核实属于沟通状态，不改变候选人的筛选分类，但会阻止重复加入联系清单。普通待定只保留最近一次完整扫描结果，首次普通淘汰只进入运行摘要，曾经推荐、AI 淘汰或已有业务记录的候选人继续保留供复核
