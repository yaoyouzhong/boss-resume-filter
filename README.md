...
![本地优先的数据流：默认在本机解析、筛选和保存，外部 AI 服务仅在用户开启后调用](diagram/local-first-data-flow/local-first-data-flow.svg)

实线表示默认本机处理路径，虚线表示用户主动开启的可选 AI 路径。完整的数据边界、凭据和漏洞报告说明见 [安全策略](SECURITY.md)。
...
**新增功能**
- **分批验证与补交**：优先提交已就绪证书，待核对记录不再阻塞其他记录；核对完成后可继续补交，自动跳过已提交及已完成记录。
- **重复证书提醒**：按证书类型和编号提示疑似重复，保留记录供核对，修改或移除后自动更新提醒。
- **识别停止**：识别时可直接在第 1 步按钮停止本轮识别，保留已完成结果。

**体验优化**
- **关键信息优先识别**：优先识别学历与学位证书的姓名、编号和类型，学校、专业不清晰时留空，减少附属信息反复识别带来的等待。
- **人工复核与修改保护**：明确提示需要核对的关键信息，重新识别保留人工修正值，并提示差异。
- **识别等待控制**：显示识别阶段和耗时，限制单份证书的累计等待时间，超时后可人工填写或重新识别。
- **队列与流程操作**：支持 Ctrl+A 全选和所选记录批量识别、验证；简化三步按钮，识别提示统一中文并支持展开查看，优化操作区间距及截图提示。
- **连续核验与页面恢复**：多批提交复用同一 Chrome，长时间等待扫码时持续检查结果，短暂连接故障自动尝试恢复；页面关闭后可重新提交未完成记录。

**问题修复**
- **结果与证书对应**：加强结果页和截图的姓名、编号及证书类型检查，修改关键信息后解除旧页面关联，避免沿用不对应的结果。

### v2.33.3 证书识别与截图体验优化

**新增功能**
- **单份结果截图**：右键证书可单独保存或重新截图，沿用保存位置。

**体验优化**
- **多页核验加载**：多份证书核验时支持同时加载两个查询页，减少页面打开等待。
- **队列快捷删除**：支持按 Del 移除选中的一份或多份证书，保留原始文件。
- **识别进度与提示**：扫描 PDF 显示逐页识别进度，识别失败时提供更明确的原因。
- **模型名称显示**：补充 DeepSeek 和 MiniMax 的中文服务商名称。

**问题修复**
- **证书识别中断**：修复部分模型因输出受限导致方向判断或字段识别中断的问题，并支持自动重试。
- **PDF 手动旋转**：修复扫描 PDF 重新识别时未采用用户手动旋转方向的问题。

### v2.33.2 模型图片能力检测与证书核验优化

**新增功能**
- **模型图片能力检测**：BOSS 主程序与独立核验助手在保存、切换或手动测试模型时检测图片识别能力，分别显示连接结果和图片测试结果；已验证的能力在重启后继续生效。

**问题修复**
- **证书编号复核**：不再仅因证书编号不足 18 位要求人工复核，减少对有效证书的误提示。
- **核验提示显示**：修复多条中文提示在不同窗口宽度下换行不完整的问题。
