...
> 当前发布版本：v2.16.1 配置体验与学历核验修复（版本号 v2.16.1）
...
- **打招呼队列**: 对符合要求的候选人集中排队发送，支持单人快捷发送、多选加入队列、暂停和失败重试
...
- **反馈原因结构化**：标记候选人反馈时可选择结构化原因，保留备注补充说明，便于后续按岗位复盘
- **岗位级复盘**：数据统计页新增岗位复盘入口，按岗位汇总反馈分布、原因 Top 项、误推/误杀原因和调整建议
...
> 开启 AI 辅助评估后，LLM 会对候选人做二次评分（±15 分调整），调整后的分数重算推荐等级，直接影响打招呼决策。
