...
> 当前发布版本：v2.18 候选人待办、状态体检与打招呼队列优化（版本号 v2.18）
...
- **今日待办**：新增候选人待办窗口，每位候选人只进入一个最高优先级任务；待复核可按学历形式、求职状态、评分等主要原因展开查看，其他任务按发送确认、已回复待推进、待完成简历评估、待打招呼和已打招呼待跟进展示下一步动作
...
- **多选模型批量保存**：系统设置中选择多个模型后，点击保存可一次性将全部选中模型加入已保存列表，此前仅保存其中一个；保存只维护模型库，不切换当前正在使用的模型，需要切换时在“使用中的模型”中选择
