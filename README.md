...
- **结果更新**：结果页支持推荐候选人、复核通过、待复核、淘汰记录和全部记录范围；复核通过包含学历等硬条件待确认后人工通过，以及 55-64 分待定候选人人工通过，评分和推荐指数保持不变。AI 评估调用失败只在 AI 评估列和候选人详情中提示，规则分和资格已通过时不要求额外复核，也不阻止加入联系清单。发送待核实属于沟通状态，不改变候选人的筛选或复核分类，但会阻止重复加入联系清单。普通待定只保留最近一次完整扫描结果，首次普通淘汰只进入运行摘要，曾经推荐、AI 淘汰或已有业务记录的候选人继续保留供复核
