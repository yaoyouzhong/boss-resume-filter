...
证书预览、证书 PDF 文本提取及简历 PDF 导入统一使用 `pypdfium2`。
两个 Windows 产物均排除 `pdfminer`、`pypdf`、`PyMuPDF` 和本地 OCR 运行库；
扫描证书转为整页图片后沿用大模型识别，扫描简历仍按文本不足处理。
依赖升级后应同时验证文本简历、旋转与多页证书、加密/损坏文件以及原生库的打包完整性。

主程序必须保留 `PIL.WebPImagePlugin` 和 `PIL._webp`，与证书导入支持的 WebP 格式一致。
打包后自动运行 `--certificate-image-smoke-test`，离线验证 WebP、PNG、JPEG、BMP
读取及识别请求图片生成；失败时停止后续打包交付，不调用大模型或读取用户资料。

独立学历工具额外排除帮助、调试模块及 Tk 演示资源，保留 FreeType 字体支持。
构建脚本先生成 spec，再仅过滤 `_tk_data/demos/`；Tk 运行库、编码和主题资源保持完整。
...
**方式 B：自动更新（已安装用户）**
- 程序启动后及运行期间定时检查版本，Gitee 优先、GitHub 复核和备选；更新提醒显示在首页
- 系统设置可开启空闲时自动下载 ZIP，默认关闭；已校验的安装包会保留，关闭升级窗口不打断下载
- 用户确认安装且招聘任务已结束后替换 .app，完成后自动重启应用
