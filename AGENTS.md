...
- **联系清单**：GUI 的所有联系动作统一进入“联系候选人”工作台，不提供绕过清单的立即发送入口。加入前过滤已沟通、硬性条件待确认、低于 55 分、人工反馈为误推/放弃或状态不适合发送的候选人，并给出跳过汇总。55-64 分的普通待定候选人允许人工确认后加入联系清单；仅 AI 评估失败但规则分和资格已通过的候选人也可由用户基于现有资料明确批准联系，不强制重跑 AI。人工批准必须持久化并在发送前复核时继续有效；自动生成联系清单仍只处理推荐及以上且没有未处理 AI 失败的候选人。工作台支持暂停、移除、失败重试，以及对待核实记录明确确认已发送或未发送
...
- 跟进状态：`followup_status`（未沟通/已打招呼/已回复/待约面/已约面/不合适/已归档）、`followup_note`、`followup_updated_at`、`next_followup_at`；打招呼成功默认安排次日跟进，已回复立即处理，待约面默认次日，不合适/已归档清除提醒；“已回复/待约面/已约面”意味着已经发生沟通，必须同步已打招呼事实；加入黑名单时结束活跃跟进并清除提醒；去重时按最新跟进时间整组保留
...
- 结果表不静默截断候选人；显示当前数量/筛选后总数，并支持“推荐候选人 / 人工确认 / 待复核 / 淘汰记录 / 全部记录”切换。推荐候选人要求评分 ≥65 且无未解决的人工确认或 AI 失败；“人工确认”只包含 55-64 分且已明确批准联系、同时不存在硬性条件待确认的候选人。发送待核实属于沟通状态，仍保留原筛选分类，但禁止再次加入联系清单
- 筛选结论、复核状态、人工联系批准和沟通状态必须独立派生：分数/资格决定推荐、待定或淘汰，尚未解决的证据不足和 AI 失败决定是否待复核，人工批准只决定普通待定候选人能否联系且不得修改评分或推荐指数，发送与跟进字段只决定沟通状态。结果范围只过滤表格，顶部统计固定使用当前岗位和日期范围
