...
- `release_flow.py` 是正常发布的唯一用户入口；`release_prepare.py`、`release_delivery.py`、`pr_delivery.py` 和 `release_dispatch.py` 仅作为确定性底层与故障恢复入口。任一阶段失败必须保留可恢复状态；发布状态必须记录 Actions run、每阶段开始/结束/耗时/尝试次数、逐附件下载/上传状态和脱敏错误。GitHub 下载使用环境代理，默认按 1 MiB 分段、总计最多 4 路下载；片段独立重试并持久化校验记录，读取超时与片段总时限共同限制慢速停滞，整包 size/SHA256 校验通过后才上传。不支持 Range 时回退普通续传。附件校验完成后立即进入 Gitee 最多 2 路直连上传队列，与下一个附件下载形成流水线；上传状态必须区分 queued 与实际占用工作线程的 uploading，并实时记录完成/失败；2026-09-11 对 v2.33.1 四个真实附件按 1/2/2/1 路交叉实测：单路平均 356.984 秒、两路平均 139.055 秒，四轮均无连接失败或重试，因此默认上传并发更新为 2；保留单路回退，不扩展为无界并发。续跑时先验证已完成阶段的后置条件，满足则跳过，不能从头重复所有远端操作；GitHub 已公开后不得回滚，只能幂等续跑 Gitee、清单同步和公开验收。Windows 项目代码启动 Git、Python、Ruff、PyInstaller、GitHub CLI、WMIC、taskkill 等控制台子进程必须统一经过 `subprocess_utils.py`：发布/测试进程复用现有控制台，使 Git/gh 后代继承同一终端且不再新建窗口；无控制台 GUI 才使用 `CREATE_NO_WINDOW` 并回放输出。禁止为发布 CLI 使用 `CREATE_NEW_CONSOLE`，Windows Terminal 会把所谓隐藏控制台激活到前台；确需显示的 Chrome 等图形程序必须显式声明 `show_window=True`。外层编排器也必须使用 `windowsHide`、`Start-Process -WindowStyle Hidden` 或等价方式启动顶层命令
...
- **PIL**：精确收集图像、字体和 Tk 预览组件；证书支持 WebP，必须保留 `WebPImagePlugin` / `_webp`，仅排除不支持的 `_avif`
...
- BOSS 与独立核验工具保存模型、切换主模型、单独指定学历核验模型或恢复跟随时，共用后台图片能力测试：仅发送程序生成的随机数字图，答案正确才确认支持，网络失败或无效回答只表示尚未确认。`vision_capability.py` 负责合成图测试与不含凭据、图片、响应正文的能力证据缓存；按服务商、接口、模型和协议参数隔离，实际图片识别返回有效高置信度字段也可补充正向证据。文字 PDF 和普通连通性成功不得确认视觉能力；已确认记录优先于模型名称启发式，重启后继续生效。
