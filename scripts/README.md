...
- 根目录 `subprocess_utils.py`：Windows 项目的统一子进程代理；发布和测试从现有控制台运行时，Git、Python、Ruff、PyInstaller、GitHub CLI 及其认证、Git、telemetry 后代全部继承同一终端，不再创建新窗口；打包后的无控制台 GUI 启动 WMIC、taskkill、更新器等命令时才使用 `CREATE_NO_WINDOW`。stdout/stderr 由父进程回放，GitHub CLI 同时关闭额外 telemetry/时区探测，Chrome 等需要展示界面的程序显式放行。
