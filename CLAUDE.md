...
- 结果表不静默截断候选人；显示当前数量/筛选后总数，并支持“推荐候选人 / 待复核 / 淘汰记录 / 全部记录”切换。推荐候选人要求评分 ≥65 且无人工确认、AI 失败或发送待确认阻断
- 状态列显示多段业务标记（跟进状态/需人工确认/反馈/屏蔽），不暴露内部发送能力；状态、学校、公司列支持 tooltip 显示完整内容
- **多选右键菜单**：支持 Ctrl/Shift 多选候选人，右键显示批量操作：加入打招呼队列、移除选中、导出选中；单选时显示完整菜单（查看详情、导入简历、打招呼、更新跟进、标记反馈、加入/移出黑名单、移除此人、导出选中）
