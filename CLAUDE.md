...
- 筛选结论、复核状态、人工联系批准和沟通状态必须独立派生：分数/资格决定推荐、待定或淘汰，尚未解决的硬条件证据不足和待定分数决定是否待复核，人工复核通过记录复核结论，联系批准只决定普通待定候选人能否联系且不得修改评分或推荐指数，发送与跟进字段只决定沟通状态。AI 评估调用失败不是复核原因或联系阻断条件。结果范围只过滤表格，顶部统计固定使用当前岗位和日期范围
