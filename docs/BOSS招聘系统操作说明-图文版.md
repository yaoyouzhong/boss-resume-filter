...
启动成功后，窗口左下角会显示当前版本号，例如 `v2.14.2`。
...
按 Ctrl 或 Shift 多选候选人后右键，可以批量打招呼、批量移除或批量导出。有发送信息的候选人可直接沟通；缺少发送信息时，系统会按岗位分组并提示切换到对应推荐页面。
...
**简历二次评估**：对已有候选人导入 PDF 或 Word 格式的简历文件后，系统会基于完整简历再次调用 LLM 进行评估，产出一个调整分并叠加到最终匹配分中。三次评估叠加公式：`最终分 = 规则评分 + AI 调整 + 简历评估调整`。评估结果和理由会显示在结果列表、详情弹窗和 Excel 导出中。如需撤回，可在右键菜单中选择撤回简历评估。
