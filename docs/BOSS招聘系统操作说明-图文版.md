...
- 对符合阈值的候选人自动打招呼，也可多选候选人批量沟通、移除或导出。
...
AI 辅助评估的读取超时时间可以在运行控制页调整。官方 API 默认 60 秒，中转服务默认 120 秒；如果模型经常慢返回，可以适当调高，但不要把网络、额度或模型不可用问题误当成岗位规则问题。
...
运行完成、主动停止或发生异常后，运行控制页的“本轮结果摘要”会保留本次结果：包括完成状态、候选人数量、筛选分布和需要处理的提示。运行日志用于查看过程细节；摘要用于快速判断本轮是否完整结束。账号没有已发布职位、页面不匹配等运行前阻断也会在弹窗中明确说明原因。
...
在候选人行上双击，可以打开详情弹窗，查看完整的评分拆解（含 AI 调整分和简历评估调整分）、评分解释、命中证据、跟进状态和人工反馈。
...
- **推荐候选人**：65 分及以上，且没有人工确认、AI 评估失败或发送待确认等阻断问题的候选人。
- **待复核**：55-64 分候选人，以及需要人工确认、AI 评估失败或发送待确认的候选人。
...
| 操作 | 说明 |
| --- | --- |
| 双击候选人行 | 打开详情弹窗，查看评分拆解（含 AI 调整分和简历评估调整分）、评分解释、命中证据、跟进状态和人工反馈 |
| 刷新结果 | 从 `candidates_all.json` 重新加载 |
| 打招呼队列 | 打开队列工作台，查看待发送候选人、发送方式、执行状态和失败原因 |
| 导出 Excel | 生成或更新 `candidates_all.xlsx`，包含身份、画像、评估、跟进、原始数据 5 个区块 |
| 清空候选人 | 清理当前岗位或全部岗位候选人，操作前会备份；黑名单记录不会被清空 |
...
- 对未打招呼候选人单独打招呼。
- 加入黑名单（记录屏蔽原因，后续扫描按 `geek_id` 跨岗位跳过，统计和 Excel 导出不再计入）。
- 更新跟进（未沟通/已打招呼/已回复/待约面/已约面/不合适/已归档 + 备注）。
- 标记反馈（合适/误推/误杀/放弃 + 备注）。
- 导入简历（支持 PDF/Word，基于完整简历做二次 LLM 评估并叠加调整分）。
- 移除此人。
- 导出选中候选人。

按 Ctrl 或 Shift 多选候选人后右键，可以加入打招呼队列、批量移除或批量导出。加入队列时，系统会先跳过已沟通、需人工确认、分数不足或状态不适合自动发送的候选人；有发送信息的候选人可直接沟通，缺少发送信息时需要浏览器停留在对应岗位的推荐页面。

单独打招呼会优先使用筛选时保存的发送信息，失败时再尝试在 BOSS 当前列表页自动滚动定位候选人卡片。列表页点击后，只有按钮变为“继续沟通”或出现明确成功提示时，系统才会记录为已打招呼；如果页面状态无法确认，系统会停止本轮发送并提示到 BOSS 沟通列表核实，避免漏记或重复发送。定位失败时，先确认浏览器仍在对应岗位的推荐页面。候选人详情和打招呼确认弹窗会展示需要人工确认的资格风险，避免误操作。
...
**黑名单**：将候选人加入黑名单后，系统按 `geek_id` 跨所有岗位屏蔽该候选人。黑名单候选人在后续扫描中被跳过，数据统计和 Excel 导出也不会计入。清空候选人时，黑名单记录会被保留，不会因为误清而重新刷到同一候选人。

**简历二次评估**：对已有候选人导入 PDF 或 Word 格式的简历文件后，系统会基于完整简历再次调用 LLM 进行评估。存在简历评估结果时，最终分以 `规则评分 + 简历评估调整` 为准，不再叠加一次 AI 调整，避免重复加分或扣分。评估结果和理由会显示在结果列表、详情弹窗和 Excel 导出中。如需撤回，可在右键菜单中选择撤回简历评估。
...
学历核验用于处理候选人毕业证书图片或 PDF。系统识别证书中的姓名和证书编号，HR 仍需要人工完成验证码、手机号验证和最终核验确认。
...
5. 点击“打开学信网验证”，在浏览器中完成人工验证码和手机号验证。
...
- 多张证书需要逐张导入、识别和核对，不要把不同候选人的证书混在一次核验里。
