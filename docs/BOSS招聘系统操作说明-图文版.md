...
- **双击切换**：双击某行将其设为当前全局模型（用于 AI 评估）。
- **右键菜单**：切换模型、测试连通性、删除模型。
- **设为学历核验模型**：右键选择后，该模型专门用于学历证书识别（需要多模态视觉能力），AI 评估继续使用全局模型。未指定时学历核验与 AI 评估共用同一模型。
- **批量测试连通性**：选中多个模型后右键测试，结果会回写到列表状态列。
...
- 选择器健康检查在连接后自动执行，一切正常时不会输出日志，仅异常时提示。
