...
| 运行控制 | 连接浏览器、选择岗位、设置滚动轮次和“筛选完成”的处理方式 |
...
启动成功后，窗口左下角会显示当前版本号，例如 `v2.24.4`。
...
AI 增强返回前，如果你已经手工修改了学历、经验、年龄、地点、薪资、技能或必要条件，系统不会再用 AI 结果覆盖这些手工修改。切换岗位或重新解析后，旧的后台结果也不会覆盖当前配置。解析结果中的技能列表会显示来源，必要条件支持按简单匹配、OR、AND 三种方式维护；有原文出处时，鼠标悬停在对应列或条件上可以查看来源片段，便于核对系统为什么这样解析。
...
当前资格审查分为“明确合格、需人工确认、明确淘汰”三种状态。明确自考、成教、函授等非统招学历时直接淘汰；第一学历明确为大专或专科后取得本科、专升本及本科教育不超过三年时，按统招本科硬条件淘汰；只有本科层级但缺少足够证据时转人工确认。系统会在候选人详情和联系清单跳过说明中展示原因，需人工确认的候选人不会进入发送流程。

## 九、查看筛选结果

进入左侧“筛选结果”。

下图为非最大化窗口，结果表优先保留核心筛选、评估和状态字段：

![筛选结果页（非最大化窗口）](assets/user-guide/05-results.png)

窗口最大化且表格空间足够时，会进一步显示毕业学校和最近公司。下图为最大化结果表右侧字段的局部图：
...
| 学历、年龄、求职状态 | 表格空间足够时显示的候选人画像字段 |
| 毕业学校、最近公司 | 仅在窗口最大化且表格空间足够时显示 |
| 风险提示 | 学历形式、工作经验等证据不足时需要人工确认的风险项 |
| 技能匹配 | 命中的技能关键词 |

结果表会根据实际可用空间自适应显示：较窄窗口显示 8 个核心字段，空间增加后显示学历、年龄和求职状态；只有窗口最大化且表格宽度足够时，才显示毕业学校和最近公司。4K 显示器的非最大化窗口不会强行塞入全部 13 列，避免表头和内容挤压。
...
搜索框支持按姓名、推荐指数和状态查找，也支持匹配分条件，例如 `>=75`、`65`。按 `Esc` 清空搜索，点击刷新可恢复初始排序。
